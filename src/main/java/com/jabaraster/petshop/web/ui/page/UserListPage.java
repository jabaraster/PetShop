...
import jabara.wicket.DeleteLinkColumn;
import jabara.wicket.EditLinkColumn;
import jabara.wicket.LinkPanel;
...
import com.jabaraster.wicket_jpa.DateTimeColumn;
...
public class UserListPage extends AdministrationPageBase {
    ...
    private Form<?> getRowCountPerPageForm() {
        if (this.rowCountPerPageForm == null) {
            this.rowCountPerPageForm = new Form<>("rowCountPerPageForm"); //$NON-NLS-1$
    ...
    private AjaxFallbackDefaultDataTable<EUser, String> getUsers() {
        if (this.users == null) {
            ...
            columns.add(AttributeColumn.<EUser> sortable(EUser.getMeta(), EntityBase_.id));
            columns.add(AttributeColumn.<EUser> sortable(EUser.getMeta(), EUser_.userId));
            columns.add(AttributeColumn.<EUser> sortable(EUser.getMeta(), EUser_.administrator));
